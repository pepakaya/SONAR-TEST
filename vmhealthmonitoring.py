...
class HealthMonitor:
    ...
    def generate_alarms(self, alarm_type, percentage, time_stamp, additional_text):
        ...
        if percentage > int(config['THRESHOLDS']['critical_threshold']):
            ...
            log_text = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - {severity} - {alarm_type} Threshold is > 90%$ {alarm_num} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            self.agent_logger.info(log_text)

            log_text1 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - MAJOR - {alarm_type} Threshold is optimal$ {config[alarm_type]['MAJOR']} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            ...
            log_text = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - {severity} - {alarm_type} Threshold is > 80%$ {alarm_num} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            self.agent_logger.info(log_text)

            log_text1 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - CRITICAL - {alarm_type} Threshold is optimal$ {config[alarm_type]['CRITICAL']} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            self.agent_logger.info(log_text1)

            log_text2 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - MINOR - {alarm_type} Threshold is optimal$ {config[alarm_type]['MINOR']} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            ...
            log_text = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - {severity} - {alarm_type} Threshold is > 70%$ {alarm_num} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            self.agent_logger.info(log_text)

            log_text1 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - MAJOR - {alarm_type} Threshold is optimal$ {config[alarm_type]['MAJOR']} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            self.agent_logger.info(log_text1)

            log_text2 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - CRITICAL - {alarm_type} Threshold is optimal$ {config[alarm_type]['CRITICAL']} $ {alarm_type} Current Percentage is {percentage} {additional_text}"
            self.agent_logger.info(log_text2)
    ...
    def generate_user_password_expiry_alarm(self, alarm_type, username, expiring_in, additional_text):
        ...
        if expiring_in <= int(config['THRESHOLDS']['password_expiry_critical_threshold']):
            severity = 'CRITICAL'
            alarm_num = config[alarm_type]['CRITICAL']
            log_text = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - {severity} - User Password is set to expire in the next {config['THRESHOLDS']['password_expiry_critical_threshold']} days$ {alarm_num} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text)

            log_text1 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - MAJOR - User Password is expiring time is optimal$ {config[alarm_type]['MAJOR']} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text1)

            log_text2 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - MINOR - User Password is expiring time is optimal$ {config[alarm_type]['MINOR']} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text2)

        elif expiring_in <= int(config['THRESHOLDS']['password_expiry_major_threshold']):
            severity = 'MAJOR'
            alarm_num = config[alarm_type]['MAJOR']
            log_text = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - {severity} - User Password is set to expire in the next {config['THRESHOLDS']['password_expiry_major_threshold']} days$ {alarm_num} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text)

            log_text1 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - CRITICAL - User Password is expiring time is optimal$ {config[alarm_type]['CRITICAL']} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text1)
            ...
            severity = 'MINOR'
            alarm_num = config[alarm_type]['MINOR']
            log_text = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - {severity} - User Password is set to expire in the next {config['THRESHOLDS']['password_expiry_minor_threshold']} days$ {alarm_num} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text)

            log_text1 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - MAJOR - User Password is expiring time is optimal$ {config[alarm_type]['MAJOR']} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text1)

            log_text2 = f"{time_stamp} {config['VM Name']['Vm_name']}$ {alarm_type} - CRITICAL - User Password is expiring time is optimal$ {config[alarm_type]['CRITICAL']} $ {username} Password is set to expire in the next {expiring_in} days {additional_text}"
            self.agent_logger.info(log_text2)
...
health_monitor = HealthMonitor('vmHealthMonitoring.log', 'alarmLog')
